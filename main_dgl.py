...
class DGLGraphConv(torch.nn.Module):
    ...
    def forward(self, graph, feat, weight=None, edge_weight=None):
        ...
        with graph.local_scope():
            ...
            feat_prodsrc = torch.tanh(torch.matmul(torch.cat((feat_src, torch.ones([feat_src.shape[0],1])),1), self.w2))
...
def train(model, device, loader, optimizer, task_type):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        if "classification" in task_type: 
            ...
            loss = reg_criterion(pred.to(torch.float32)[is_labeled], labels.to(torch.float32)[is_labeled])
        loss.backward()
        optimizer.step()
