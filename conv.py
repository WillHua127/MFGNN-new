...
class GNN_node(torch.nn.Module):
    ...
    """
    def __init__(self, num_layer, emb_dim, device, drop_ratio = 0.5, JK = "last", residual = False, gnn_type = 'gin'):
        '''
            emb_dim (int): node embedding dimensionality
    ...
    def forward(self, x, edge_index, edge_attr=None):
        ...
        for layer in range(self.num_layer):
            ...
            if layer == self.num_layer - 1:
                ...
                h = [F.dropout(h_i, self.drop_ratio, training = self.training) for h_i in h]
            else:
                #h = F.dropout(F.relu(h), self.drop_ratio, training = self.training)
                h = [F.dropout(F.relu(h_i), self.drop_ratio, training = self.training) for h_i in h]
